...
int main(int argc, char* argv[])
{
    ...
    try
    {
        ...
        desc.add_options()
            ...
            ("model,m", po::value<fs::path>(&modelfile)->required()->default_value("../../bfm2017/bfm2017-1_bfm_nomouth.bin"),//"../../share/sfm_shape_3448.bin"),
            ...
            ("image,i", po::value<fs::path>(&imagefile)->required()->default_value("../../data/1.png"),
                "an input image")
            ("landmarks,l", po::value<fs::path>(&landmarksfile)->required()->default_value("../../data/1.pts"),
                "2D landmarks for the image, in ibug .pts format")
            ("mapping,p", po::value<fs::path>(&mappingsfile)->required()->default_value("../../bfm2017/ibug_to_bfm2017-1_bfm_nomouth.txt"),//"../../share/ibug_to_sfm.txt"),
                "landmark identifier to model vertex number mapping")
            ("model-contour,c", po::value<fs::path>(&contourfile)->required()->default_value("../../bfm2017/bfm2017-1_bfm_nomouth_model_contours.json"),//"../../share/sfm_model_contours.json"),
                "file with model contour indices")
            ("output,o", po::value<fs::path>(&output_basepath)->required()->default_value(""),
	...
	if (output_basepath.string().compare("") == 0) {
		output_basepath = imagefile.root_path().string() + imagefile.parent_path().string() + "/" + imagefile.stem().string() + ".ceres";
	}
	...
	// define the number of 3dmm coefficients to be used
    const int num_shape_coeffs = 160;
    const int num_blendshape_coeffs = 80;
	const int num_color_coeffs = 160;

	//std::vector<eos::morphablemodel::Blendshape> blendshapes = eos::morphablemodel::load_blendshapes(blendshapesfile.string());
    std::vector<eos::morphablemodel::Blendshape> blendshapes = cz::util::expression_pca_to_blendshapes(morphable_model.get_expression_model().value(), num_blendshape_coeffs);
    ...
    shape_coefficients.resize(num_shape_coeffs); // Todo: Currently, the value '10' is hard-coded everywhere. Make it dynamic.
    std::vector<double> blendshape_coefficients;
    blendshape_coefficients.resize(num_blendshape_coeffs);
    ...
    for (int i = 0; i < image_points.size(); ++i)
    {
        CostFunction* cost_function =
            new AutoDiffCostFunction<fitting::LandmarkCost, 2 /* num residuals */,
                                     ...
                                     num_shape_coeffs /* shape-coeffs */, num_blendshape_coeffs /* bs-coeffs */>(
                new fitting::LandmarkCost(morphable_model.get_shape_model(), blendshapes, image_points[i],
                                          vertex_indices[i], image.cols, image.rows, use_perspective, num_shape_coeffs, num_blendshape_coeffs));
    ...
    CostFunction* shape_prior_cost =
        new AutoDiffCostFunction<fitting::PriorCost, num_shape_coeffs /* num residuals */,
                                 num_shape_coeffs /* shape-coeffs */>(
            new fitting::PriorCost(num_shape_coeffs, 35.0));
    ...
    CostFunction* blendshapes_prior_cost =
        new AutoDiffCostFunction<fitting::PriorCost, num_blendshape_coeffs /* num residuals */,
                                 num_blendshape_coeffs /* bs-coeffs */>(
            new fitting::PriorCost(num_blendshape_coeffs, 10.0));
    ...
    colour_coefficients.resize(num_color_coeffs);
    ...
    for (int i = 0; i < morphable_model.get_shape_model().get_data_dimension() / 3; ++i)
    {
        CostFunction* cost_function =
            new AutoDiffCostFunction<fitting::ImageCost, 3 /* Residuals: [R, G, B] */,
                                     4 /* camera rotation (quaternion) */,
                                     num_cam_trans_intr_params /* camera translation & focal length */,
                                     num_shape_coeffs /* shape-coeffs */, num_blendshape_coeffs /* bs-coeffs */, num_color_coeffs /* colour coeffs */>(
                new fitting::ImageCost(morphable_model, blendshapes, image, i, use_perspective, num_shape_coeffs, num_blendshape_coeffs, num_color_coeffs));
        fitting_costfunction.AddResidualBlock(cost_function, NULL, &camera_rotation[0],
                                              &camera_translation_and_intrinsics[0], &shape_coefficients[0],
    ...
    CostFunction* colour_prior_cost =
        new AutoDiffCostFunction<fitting::PriorCost, num_color_coeffs /* num residuals */, num_color_coeffs /* colour-coeffs */>(
            new fitting::PriorCost(num_color_coeffs, 35.0));
    ...
    core::write_obj(mesh, output_basepath.string() + ".obj");
    ...
    render::draw_wireframe(outimg, mesh, t_mtx * rot_mtx, projection_mtx,
                           fitting::get_opencv_viewport(image.cols, image.rows));

    cv::imwrite(output_basepath.string() + ".png", outimg);
