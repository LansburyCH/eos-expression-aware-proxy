 ...
 *
 * See fit_shape_model(cv::Mat, eos::morphablemodel::MorphableModel,
 * std::vector<eos::morphablemodel::Blendshape>, std::vector<cv::Vec2f>, std::vector<int>, float lambda) for a
 * simpler overload that just returns the shape instance.
 *
 * @param[in] affine_camera_matrix The estimated pose as a 3x4 affine camera matrix that is used to fit the
 * shape.
 * @param[in] morphable_model The 3D Morphable Model used for the shape fitting.
 * @param[in] blendshapes A vector of blendshapes that are being fit to the landmarks in addition to the PCA
 * model.
 ...
 * @param[in] num_coefficients_to_fit How many shape-coefficients to fit (all others will stay 0). Should be
 ...
 * @param[out] blendshape_coefficients Output parameter that will contain the resulting blendshape
 * coefficients.
                                 ...
                                 std::vector<float>& blendshape_coefficients)
{
    ...
    do // run at least once:
    {
        ...
        std::abs(
            Eigen::Map<const VectorXf>(current_blendshape_coeffs.data(), current_blendshape_coeffs.size())
                .norm() -
            Eigen::Map<const VectorXf>(last_blendshape_coeffs.data(), last_blendshape_coeffs.size())
                .norm()) >= 0.01);
 ...
 * @param[in] face_instance A shape instance from which the expression coefficients should be estimated (i.e.
 * the current mesh without expressions, e.g. estimated from a previous PCA-model fitting).
 ...
 * @param[in] lambda_expressions The regularisation parameter (weight of the prior towards the mean). Only
 * used for expression-PCA fitting.
 * @param[in] num_expression_coefficients_to_fit How many expression coefficients to fit (all others will stay
 * 0). Should be bigger than zero, or std::nullopt to fit all coefficients. Only used for expression-PCA
 * fitting.
 ...
 * std::vector<morphablemodel::Blendshape>&, const core::LandmarkCollection<cv::Vec2f>&, const
 * core::LandmarkMapper&, int, int, const morphablemodel::EdgeTopology&, const fitting::ContourLandmarks&,
 * const fitting::ModelContour&, int, cpp17::optional<int>, float). for a simpler overload with reasonable
 * defaults and no optional output.
 *
 ...
 * @param[in] model_contour The model contour indices that should be considered to find the closest
 ...
 * @param[in] num_shape_coefficients_to_fit How many shape-coefficients to fit (all others will stay 0).
 * Should be bigger than zero, or std::nullopt to fit all coefficients.
 * @param[in] lambda_identity Regularisation parameter of the PCA shape fitting.
 * @param[in] num_expression_coefficients_to_fit How many shape-coefficients to fit (all others will stay 0).
 * Should be bigger than zero, or std::nullopt to fit all coefficients. Only used for expression-PCA fitting.
 * @param[in] lambda_expressions Regularisation parameter of the expression fitting. Only used for
 * expression-PCA fitting.
 * @param[in] initial_rendering_params Currently ignored (not used).
 * @param[in,out] pca_shape_coefficients If given, will be used as initial PCA shape coefficients to start the
 * fitting. Will contain the final estimated coefficients.
 * @param[in,out] expression_coefficients If given, will be used as initial expression blendshape coefficients
 * to start the fitting. Will contain the final estimated coefficients.
 * @param[out] fitted_image_points Debug parameter: Returns all the 2D points that have been used for the
 * fitting.
 * @return The fitted model shape instance and the final pose.
 */
inline std::tuple<core::Mesh, fitting::RenderingParameters, std::vector<float>, std::vector<float>>
fit_shape_and_pose(const morphablemodel::MorphableModel& morphable_model,
                   ...
                   std::vector<Eigen::Vector2f>& fitted_image_points, bool fix_pca_shape_coeffs = false,
                   bool fix_expression_coeffs = false)
{
    ...
    {
        if (cpp17::holds_alternative<eos::morphablemodel::PcaModel>(
                morphable_model.get_expression_model().value()))
        {
            expression_coefficients.resize(
                cpp17::get<eos::morphablemodel::PcaModel>(morphable_model.get_expression_model().value())
                    .get_num_principal_components());
        } else if (cpp17::holds_alternative<eos::morphablemodel::Blendshapes>(
                       morphable_model.get_expression_model().value()))
        {
            expression_coefficients.resize(
                cpp17::get<eos::morphablemodel::Blendshapes>(morphable_model.get_expression_model().value())
                    .size());
        }
    }
    ...
    assert(morphable_model.has_separate_expression_model()); // Note: We could also just skip the expression
    ...
    return {current_mesh, rendering_params, pca_shape_coefficients,
            expression_coefficients}; // I think we could also work with a VectorXf face_instance in
 ...
 * std::pair<render::Mesh, fitting::RenderingParameters> fit_shape_and_pose(const
 * morphablemodel::MorphableModel&, const std::vector<morphablemodel::Blendshape>&, const
 * core::LandmarkCollection<cv::Vec2f>&, const core::LandmarkMapper&, int, int, const
 * morphablemodel::EdgeTopology&, const fitting::ContourLandmarks&, const fitting::ModelContour&, int,
 * cpp17::optional<int>, float, cpp17::optional<fitting::RenderingParameters>, std::vector<float>&,
 * std::vector<float>&, std::vector<cv::Vec2f>&)
 *
 * Todo: Add a convergence criterion.
 *
 * \p num_iterations: Results are good for even a single iteration. For single-image fitting and
 ...
 * @return The fitted model shape instance and the final pose.
 */
inline auto fit_shape_and_pose(
    const morphablemodel::MorphableModel& morphable_model,
    ...
    std::vector<float>& init_pca_shape_coeffs, std::vector<float>& init_expression_coeffs,
    bool fix_pca_shape_coeffs = false, bool fix_expression_coeffs = false, int num_iterations = 5,
    cpp17::optional<int> num_shape_coefficients_to_fit = cpp17::nullopt, float lambda_identity = 50.0f,
    ...
    cpp17::optional<float> lambda_expressions = cpp17::nullopt)
{
    ...
    return fit_shape_and_pose(morphable_model, landmarks, landmark_mapper, image_width, image_height,
							  ...
							  init_pca_shape_coeffs, init_expression_coeffs, fitted_image_points, 
							  fix_pca_shape_coeffs, fix_expression_coeffs);
};
    ...
    std::vector<Eigen::Vector2f>& fitted_image_points)
{
    ...
    // The 2D and 3D point correspondences used for the fitting:
    vector<Vector4f> model_points; // the points in the 3D shape model
    // Get the model points corresponding to the given image points (mean if given no initial coeffs, from the
    // computed shape otherwise):
