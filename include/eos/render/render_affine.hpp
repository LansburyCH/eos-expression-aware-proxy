 ...
 * The camera matrix should be one estimated with fitting::estimate_affine_camera (Hartley & Zisserman
 * algorithm).
 *
 * If the given mesh is a shape-only mesh without vertex-colour information, the vertices will be rendered in
 * grey.
 *
 ...
 * optimised for only the extraction.
 *
...
inline std::pair<core::Image4u, core::Image1d> render_affine(const core::Mesh& mesh,
                                                             Eigen::Matrix<float, 3, 4> affine_camera_matrix,
                                                             int viewport_width, int viewport_height,
                                                             bool do_backface_culling = true, Eigen::Matrix<float, 4, 4> *model_view_matrix = 0, core::Image3d* coordsbuffer = 0, core::Image2d* texcoordsbuffer = 0)
{
    ...
    // assert(mesh.vertices.size() == mesh.texcoords.size() || mesh.texcoords.empty()); // same for the
    // texcoords
	...
	// CZ: get vertices 3d coordinates before projection
	vector<Eigen::Vector4f> vertices_coords_3d_before_projection;
	if (model_view_matrix != 0) {
		vertices_coords_3d_before_projection.reserve(mesh.vertices.size());
	}
    ...
    for (int i = 0; i < mesh.vertices.size(); ++i)
    {
		// CZ
		if (model_view_matrix != 0) {
			vertices_coords_3d_before_projection.push_back(*model_view_matrix * Eigen::Vector4f(mesh.vertices[i][0], mesh.vertices[i][1], mesh.vertices[i][2], 1.0f));
		}


        const Eigen::Vector4f vertex_screen_coords =
            affine_with_z *
            Eigen::Vector4f(mesh.vertices[i][0], mesh.vertices[i][1], mesh.vertices[i][2], 1.0f);
        const glm::tvec4<float> vertex_screen_coords_glm(vertex_screen_coords(0), vertex_screen_coords(1),
                                                         vertex_screen_coords(2), vertex_screen_coords(3));
        ...
        projected_vertices.push_back(
            detail::Vertex<float>{vertex_screen_coords_glm, vertex_colour,
                                  glm::tvec2<float>(mesh.texcoords[i][0], mesh.texcoords[i][1])});
    }
    ...
    for (const auto& tri_indices : mesh.tvi)
    {
		...
		// CZ
		t.v0_index = tri_indices[0];
		t.v1_index = tri_indices[1];
		t.v2_index = tri_indices[2];
    ...
    {
		// CZ
		vector<Eigen::Vector4f> tri_vertices_coords_3d_before_projection;
		vector<Eigen::Vector4f>* tri_vertices_coords_3d_before_projection_ptr;
		if (coordsbuffer != 0) {
			tri_vertices_coords_3d_before_projection.push_back(vertices_coords_3d_before_projection[triangle.v0_index]);
			tri_vertices_coords_3d_before_projection.push_back(vertices_coords_3d_before_projection[triangle.v1_index]);
			tri_vertices_coords_3d_before_projection.push_back(vertices_coords_3d_before_projection[triangle.v2_index]);
			tri_vertices_coords_3d_before_projection_ptr = &tri_vertices_coords_3d_before_projection;
		}
		else {
			tri_vertices_coords_3d_before_projection_ptr = 0;
		}


		// CZ: change of function interface
		detail::raster_triangle_affine(triangle, colourbuffer, depthbuffer, tri_vertices_coords_3d_before_projection_ptr, coordsbuffer, texcoordsbuffer);
    }
